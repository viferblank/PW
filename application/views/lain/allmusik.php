...
<link rel='stylesheet' type='text/css' href='<?php echo base_url(); ?>/css/full.css'/>
	...
	<?php
			...
			echo $this->msk->msg('msgHapusMusik','spanO');
...
<table align='center' width='100%' border='0'>
		...
		<div class='spanC'>
		<audio>
				<source src="<?php echo base_url(); ?>/musik/<?php echo $row->file; ?>" type="audio/mp3" preload="auto"/>Your browser does not support the audio element.
			</audio>
		</div>
		<div class='spanB'>
			<div class='spanB'>
		...
		<div class='spanB'>
<?php if(($this->session->userdata('level') == 'admin') OR ($this->session->userdata('level') == 'user') AND ($this->session->userdata('id') == $row->id_user)) { ?> <a href='<?php echo base_url();?>first/hapus_musik/<?php echo $row->id_album; ?>/<?php echo $row->file; ?>'><img src='<?php echo base_url(); ?>image/trash.png' class='l' title='Hapus' onclick="return confirm('Anda yakin ingin menghapus nya..!!')"></img></a><? } ?>
		</div>
</table>
</div>
